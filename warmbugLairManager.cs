...
public class warmbugLairManager : MonoBehaviour
{
    ...
    private Dictionary<string, GameObject> attacker_entity_collection;

    private void Awake()
    {
        catalog_bug_entity_collection = new Dictionary<string, GameObject>();
        attacker_entity_collection = new Dictionary<string, GameObject>();
        warmbug_distribution_collection = new ODM.ODMDictionary();
    }
    void Start()
    {
        //Import Warmbugs from resource XXX
        ...
        attacker_entity_collection.Add("Inuji", Resources.Load<GameObject>(@"Warmbugs\Inuji\0000\Inuji"));
        attacker_entity_collection.Add("Larvae", Resources.Load<GameObject>(@"Warmbugs\Larvae\0000\Larvae"));
        attacker_entity_collection.Add("Scorpion", Resources.Load<GameObject>(@"Warmbugs\Silencer\0000\Silencer"));
    }

    public GameObject getRandomAttacker()
    {
        return attacker_entity_collection.Values.ElementAt(Random.Range(0, attacker_entity_collection.Count));
    }

    public void setLairInfo(string _level_name, List<lairInfo> _lair_info_collection)
    {
        //Will replace value if key already exists
        warmbug_distribution_collection.add(_level_name, JsonMapper.ToJson(_lair_info_collection));
    }
    ...
    //Set all level approved(currently not used)XXX
    ...
    //    {
    //        ODMObj.event_manager.GetComponent<eventCenter>().setFlagValue(ODMVariable.convert.getWarmbugResetFlag(map_name_collection[i]), _flag_value);
    //    }
    //}
}
